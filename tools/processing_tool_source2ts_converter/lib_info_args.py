...
time_format_range = '%Y-%m-%d'
time_format_run = '%Y-%m-%d %H:%M'
time_machine = pd.Timestamp.now

# logging information
logger_name = 'hmc_source2ts_converter_logger'
logger_file = 'hmc_source2ts_converter.txt'
logger_handle = 'file'  # 'file' or 'stream'
logger_format = \
    '%(asctime)s %(name)-12s %(levelname)-8s %(message)-80s %(filename)s:[%(lineno)-6s - %(funcName)-20s()] '
