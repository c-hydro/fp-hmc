...
from tools.postprocessing_tool_ensemble_maker.lib_utils_io import read_obj, write_obj
from tools.postprocessing_tool_ensemble_maker.lib_utils_system import fill_tags2string, make_folder
from tools.postprocessing_tool_ensemble_maker.lib_info_args import time_format_algorithm
from tools.postprocessing_tool_ensemble_maker.lib_data_io_json import read_file_ts_hydrograph, write_file_ts_hydrograph
...
class DriverDynamic:
    ...
    def __init__(self, time_now, time_run, src_dict, anc_dict, dest_dict, static_data_collection,
        ...
        self.run_name = self.alg_ancillary['run_name']
        ...
        for section_name, basin_name in zip(self.section_name_list, self.basin_name_list):
            ...
            file_path_ts_src = self.define_file_timeseries(
                self.time_run, self.folder_name_ts_src, self.file_name_ts_src, file_ensemble_list=self.ensemble_list,
                file_extra_args={'section_name': section_name, 'basin_name': basin_name, 'run_name': self.run_name})
        ...
        for (point_tag, point_file_dest), point_file_anc in zip(
            ...
            if not os.path.exists(point_file_dest):
                if os.path.exists(point_file_anc):
                    ...
                    write_file_ts_hydrograph(point_file_dest, point_data)
    ...
    def organize_dynamic_data(self):
        ...
        for (point_tag, point_file_src), point_file_anc in zip(
            ...
            if not os.path.exists(point_file_anc):
                ...
                tag_ts_obs_list = []
                tag_ts_sim_list = []
                for ensemble_step, point_file_step in zip(ensemble_list, point_file_src):

                    if point_file_src.__len__() == 1:
                        tag_ts_sim_step = tag_ts_sim_srun
                    elif point_file_src.__len__() > 1:
                        tag_ts_sim_step = tag_ts_sim_mrun.format(int(ensemble_step))
                    else:
                ...
                if ts_collections is not None:
                    ...
                    columns_obs = sorted(list(set(tag_ts_obs_list)))
                    columns_sim = sorted(list(set(tag_ts_sim_list)))
                    ...
                    ts_dataframe_sim = pd.DataFrame(data=ts_collections_sim, columns=columns_sim,
                                                    index=ts_collections[tag_ts_time])
